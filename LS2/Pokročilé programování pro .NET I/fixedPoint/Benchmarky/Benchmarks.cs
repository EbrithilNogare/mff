...
namespace Benchmarky
{
    class Benchmarks
    {
         static void Main(string[] args)
        {
            ...
            reports.Add(BenchmarkRunner.Run<SubtractTestsWithoutDeclarationForCycle>());
            reports.Add(BenchmarkRunner.Run<MultiplyTestsWithoutDeclarationForCycle>());
            reports.Add(BenchmarkRunner.Run<DivideTestsWithoutDeclarationForCycle>());
    ...
    public class SubtractTestsWithoutDeclarationForCycle
    {
        Fixed<Q24_8> f1 = new Fixed<Q24_8>(512);
        Fixed<Q24_8> f2 = new Fixed<Q24_8>(42);
        Fixed<Q24_8> f3;
    ...
    public class MultiplyTestsWithoutDeclarationForCycle
    {
        Fixed<Q24_8> f1 = new Fixed<Q24_8>(512);
        Fixed<Q24_8> f2 = new Fixed<Q24_8>(42);
        Fixed<Q24_8> f3;
    ...
    public class DivideTestsWithoutDeclarationForCycle
    {
        Fixed<Q24_8> f1 = new Fixed<Q24_8>(512);
        Fixed<Q24_8> f2 = new Fixed<Q24_8>(42);
        Fixed<Q24_8> f3;
    ...
    public class GauseEliminationTests
    {
        ...
        public void Q24_8Test()
        {
            ...
            var gause = new GauseElimination<Fixed<Q24_8>>();
            gause.SolveLinearEquations(input);
        }
        [Benchmark]
        public void Q16_16Test()
        {
            ...
            var gause = new GauseElimination<Fixed<Q16_16>>();
            gause.SolveLinearEquations(input);
        }
        [Benchmark]
        public void Q8_24Test()
        {
            ...
            var gause = new GauseElimination<Fixed<Q8_24>>();
            gause.SolveLinearEquations(input);
        }
        [Benchmark]
        public void FloatTest()
        {
            ...
            var gause = new GauseElimination<float>();
            gause.SolveLinearEquations(input);
        }
        [Benchmark]
        public void DoubleTest()
        {
            ...
            var gause = new GauseElimination<double>();
            gause.SolveLinearEquations(input);
        }
    }


    public class GauseElimination<T>
    {
        public void SolveLinearEquations(T[,] M)
        {
            // input checks
            ...
            // pivoting
            for (int col = 0; col + 1 < rowCount; col++) if (M[col, col] == (dynamic)0)
                // check for zero coefficients
                {
                    // find non-zero coefficient
                    int swapRow = col + 1;
                    for (; swapRow < rowCount; swapRow++) if (M[swapRow, col] != (dynamic)0) break;

                    if (M[swapRow, col] != (dynamic)0) // found a non-zero coefficient?
                    {
                    ...
                    else return; // no, then the matrix has no unique solution
                }
            ...
            for (int sourceRow = 0; sourceRow + 1 < rowCount; sourceRow++)
            {
                for (int destRow = sourceRow + 1; destRow < rowCount; destRow++)
                {
                    ...
                    for (int i = 0; i < rowCount + 1; i++)
                    {
                        var a = (dynamic)M[destRow, i] * (dynamic)df;
                        var b = (dynamic)M[sourceRow, i] * (dynamic)sf;
                        M[destRow, i] = (T)(a - b);
                    }
                }
            }
            ...
            {
                T f = M[row, row];
                if (f == (dynamic)0) return;

                for (int i = 0; i < rowCount + 1; i++) M[row, i] = (T)(M[row, i] / (dynamic)f);
                for (int destRow = 0; destRow < row; destRow++)
                {
                    M[destRow, rowCount] = (T)(M[destRow, rowCount] - (dynamic)M[destRow, row] * (dynamic)M[row, rowCount]);
                    M[destRow, row] = (dynamic)0;
                }
            }
            return;
        }
