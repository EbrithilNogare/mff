...
namespace DavidNapravnik
{
    public class ModuleFullColor : DefaultRasterModule
    {
        ...
        /// </summary>
        public override string Tooltip => "[ignore]";
        ...
        public override void Update()
        {
            ...
            Util.TryParse(p, "ignore", ref ignoreInput);
            ...
            unsafe
            {
                ...
                if (inImage != null && !ignoreInput)
                {
                    ...
                    for (int x = 0; x < wid; x++) // shuffle blocks of pixels
                    {
                        ...
                        for (int y = 0; y < hei; y ++)
                        {
                            optr = baseOptr + (y * hei + x) * dO;
                            iptr = baseIptr + ((int)((double)y / hei * height) * width + (int)((double)x / wid * width)) * dI;
