            ...
            using (Graphics gfx = Graphics.FromImage(graph))
            {
                ...
                for (int hue = 0; hue <= (histArray.GetLength(0) - 1); hue++)
                    for (int saturation = 0; saturation <= (histArray.GetLength(1) - 1); saturation++)
                    {
                        ...
                        float x1 = (float)Math.Sin(angle) * (distance) + centerX;
                        float y1 = (float)Math.Cos(angle) * (distance) + centerY;

                        float x2 = (float)Math.Sin(angle + anglePart) * (distance) + centerX;
                        float y2 = (float)Math.Cos(angle + anglePart) * (distance) + centerY;
                        ...
                        float x4 = (float)Math.Sin(angle) * (distance + distancePart) + centerX;
                        float y4 = (float)Math.Cos(angle) * (distance + distancePart) + centerY;
