...
def differential_evolution(pop, _fits, generation, F=None, CR=None):
    if F is None:
        F = 0.8 + 0.2 * (1.0 - generation / MAX_GEN)  # Adaptive F
    if CR is None:
        CR = 0.9 - 0.5 * (1.0 - generation / MAX_GEN)  # Adaptive CR

    new_pop = []
    for target in pop:
        candidates = random.sample(pop, 5)  # Use 5 individuals for mutation
...
def crossover_operator(pop, cross_op, fits_objs, generation, cx_prob):  # Rename cross to cross_op
    off = []
    for p1, p2 in zip(pop[0::2], pop[1::2]):
        if random.random() < cx_prob:
            o1, o2 = cross_op(p1, p2, generation=generation, fits_objs=fits_objs)  # Rename cross to cross_op
        else:
            o1, o2 = p1[:], p2[:]
        off.append(o1)
        off.append(o2)
...
def evolutionary_algorithm(pop, max_gen, fitness, operators, mate_sel, mutate_ind, *, map_fn=map, log=None):
    ...
    for G in range(max_gen):
        ...
        offspring = mate(mating_pool, operators, G, fits_objs)  # Pass fits_objs to mate function
        pop = offspring[:]
...
if __name__ == '__main__':
    ...
    for fit_gen, fit_name in zip(fit_generators, fit_names):
        ...
        xover = functools.partial(crossover_operator, cross=arithmetic_cross, cx_prob=CX_PROB)
        ...
        for run in range(REPEATS):
            ...
            pop = evolutionary_algorithm(pop, MAX_GEN, fit, [differential_evolution, xover, mut],
                                          tournament_selection, mutate_ind, map_fn=map, log=log)
